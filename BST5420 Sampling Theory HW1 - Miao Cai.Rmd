...
3. I randomly scrolled to a page and randomly pick a book on that page. I repeated this procedure for 10 times and I get a SRS sample of 10 books. 
```{r echo = FALSE}
dat = data.frame(
  book_name = c(
...
5. The formula to estimate confidence interval for the population mean is $\bar{x} \pm t*\frac{s}{\sqrt{n}}$. The degrees of freedom is $n - 1 = 9$, so the t statistic is `r round(qt(0.975, 9), 2)`. The confidence interval can be calculated as follows in R:
```{r}
n = 10
t = qt(0.975, n - 1)
s = sd(dat$price)
...
round(xbar + t*s/sqrt(n), 2)
```
6. The mean price of book with keywords "Bayesian data analysis" on Amazon is about 60.636 US dollars. If we take samples for a lot of times, 95% these estimated mean price will fall between `r round(xbar - t*s/sqrt(n), 2)` and `r round(xbar + t*s/sqrt(n), 2)`.
